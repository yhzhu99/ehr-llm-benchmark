...
def query_llm(
    ...
    elif model in ['llama3', 'llama3:70b']:
        try:
    ...
    for x, y, pid, record_time, missing_mask in tqdm(zip(xs, ys, pids, record_times, missing_masks), total=len(xs)):
        ...
        if output_prompts:
            with open(os.path.join(sub_prompts_path, f'{pid}.txt'), 'w') as f:
                f.write(userPrompt)
            Path(sub_prompts_path + '_detail').mkdir(parents=True, exist_ok=True)
            with open(os.path.join(sub_prompts_path + '_detail', f'{pid}.txt'), 'w') as f:
                f.write(detail)
        if output_logits:
            try:
                # print(f'Start querying LLM patient {pid}...')
