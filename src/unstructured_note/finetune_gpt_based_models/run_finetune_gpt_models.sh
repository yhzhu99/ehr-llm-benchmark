...
LOG_DIR="logs/running_logs/finetune_gpt_lora"
...
echo "Generating all commands for GPT LoRA fine-tuning..."
for MODEL in "${GPT_MODELS[@]}"; do
    for DATASET_TASK in "${DATASET_TASK_OPTIONS[@]}"; do
        ...
        LOG_FILE="${LOG_DIR}/gpt_lora_finetune-${MODEL_SAFE_NAME}-${DATASET}-${TASK}.log"
        LOG_FILES+=("$LOG_FILE")
    done
done
...
TOTAL_RUNS=${#COMMANDS[@]}
MAX_JOBS=9 # Set the maximum number of concurrent jobs (you can adjust this, LoRA might be memory intensive)

echo "Starting GPT LoRA fine-tuning with ${TOTAL_RUNS} different configurations..."
...
echo "All GPT LoRA fine-tuning commands have been dispatched. Waiting for remaining background jobs to finish..."
wait

echo "All GPT LoRA fine-tuning completed successfully!"