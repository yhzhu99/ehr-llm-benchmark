...
for MODEL in "${ALL_MODELS[@]}"; do
    for DATASET_TASK in "${DATASET_TASK_OPTIONS[@]}"; do
        ...
        echo "Training MLP on $MODEL embeddings for $TASK task on $DATASET dataset"
        python src/unstructured_note/freeze_setting/tune_embeddings.py --model "$MODEL" --task "$TASK" --dataset "$DATASET" --batch_size 64 --learning_rate 1e-4 --epochs 50 --patience 5
    done
done
